...
<script>
        ...
        if (reprintData) {
            ...
            ticketDetails = reprintData.boletos.map(b => `
                <div class="ticket-row">
                    <span>1x ${b.tarifa_nombre}</span>
                    <span>S/ ${parseFloat(b.price).toFixed(2)}</span>
                </div>
            `).join('');
        } else {
        ...
        const ticketHTML = `
                <div class="ticket">
                    <div class="ticket-header">
                        ...
                        <div style="font-size: 10px;">Sistema de Venta de Entradas ${reprintData ? '(REIMPRESIÓN)' : ''}</div>
                    </div>
    ...
    function resetPOS() {
        ...
        tariffs.forEach(tariff => {
            const qtyEl = document.getElementById(`qty-${tariff.id}`);
        ...
        $.ajax({
            ...
            success: function(resp) {
                if (resp.status === 'success') {
                    ...
                    body.innerHTML = resp.data.map(sale => {
                        ...
                        return `
                            <tr>
                                ...
                                <td>${sale.pelicula_nombre}</td>
                                <td>${sale.showtime_hora}</td>
                                <td>S/ ${amount.toFixed(2)}</td>
                                <td>${sale.medio_pago}</td>
                                <td><span class="status-badge status-${sale.estado.toLowerCase()}">${sale.estado}</span></td>
                                <td>
                                    <button class="action-btn btn-reprint" onclick="reprintTicket(${sale.id})">🖨️</button>
                                    ${sale.estado === 'PAGADO' ? `
                    ...
                    document.getElementById('salesTotalDay').textContent = `S/ ${total.toFixed(2)}`;
                    document.getElementById('salesTotalCash').textContent = `S/ ${cash.toFixed(2)}`;
                    document.getElementById('salesTotalCard').textContent = `S/ ${card.toFixed(2)}`;
                } else {
                    body.innerHTML = `<tr><td colspan="7" style="text-align: center; color: var(--danger);">${resp.message}</td></tr>`;
                }
            },
            error: function() {
